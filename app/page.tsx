        ...
        {[{n:"Rahul",p:true},{n:"Priya",p:true},{n:"Arjun",p:false}].map(({n,p}) => (
          <div key={n} className="flex items-center justify-between bg-white/5 border border-white/8 rounded-xl px-3 py-2.5">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-[10px] text-white/50">{n[0]}</div>
              <span className="text-white/60 text-xs">{n}</span>
            </div>
            <span className={`text-xs font-semibold ${p ? "text-[#CCFF00]" : "text-white/20"}`}>{p ? "Paid ✓" : "Pending"}</span>
          </div>
        ))}
      </div>
    ),
  ...
  return (
    <main className="bg-black text-white overflow-x-hidden">
      ...
      {/* ══════════════════════════════════════════════════════════════════
          ...
          Mirrors tuyo.com's exact hero structure
      ══════════════════════════════════════════════════════════════════ */}
      <section className="relative min-h-[100svh] bg-black flex overflow-hidden">

        {/* ── Desktop right panel: floating rounded-rect photo ── */}
        <div className="hidden md:block absolute right-5 top-16 bottom-8 rounded-3xl overflow-hidden"
          style={{ left: "44%" }}>
          <Image
            src="/turfInBG.png"
            alt="TurfIn app"
          ...
          />
          {/* edge fade back to black */}
          <div className="absolute inset-y-0 left-0 w-24 bg-gradient-to-r from-black to-transparent" />
        </div>

        {/* ── Mobile full-bg photo with heavy gradient ── */}
        <div className="md:hidden absolute inset-0">
          <Image src="/turfInBG.png" alt="TurfIn" fill priority className="object-cover object-center" />
          <div className="absolute inset-0 bg-gradient-to-t from-black via-black/80 to-black/40" />
          <div className="absolute inset-0 bg-gradient-to-r from-black/90 to-transparent" />
        </div>

        {/* ── Left text content, bottom-aligned ── */}
        <div className="relative z-10 flex flex-col justify-end pb-16 px-5 md:px-10 w-full md:w-[44%]">
          {/* Live badge */}
          ...
          {/* CTA buttons — dark store-style, mirrors tuyo's App Store / Play Store */}
          <div className="flex items-center gap-3 flex-wrap">
            <Link
              href="/#waitlist"
              className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-[#111] border border-white/12 text-white text-sm font-semibold hover:bg-[#1a1a1a] hover:border-white/20 transition-all duration-200"
            >
          ...
          <div className="flex items-center gap-3 mt-6">
            <div className="flex -space-x-2">
              {AVATARS.map(({ char, src, gradient }) => (
                <div
                  key={char}
                  className={`w-7 h-7 rounded-full border-2 border-black flex items-center justify-center overflow-hidden ${
                    src ? "bg-white" : `bg-linear-to-br ${gradient}`
                  }`}
                >
                  {src ? (
                    <Image src={src} alt={char} width={28} height={28} className="object-cover w-full h-full" />
                  ) : (
                    <span className="text-[9px] font-bold text-white/80">{char}</span>
                  )}
                </div>
              ))}
            </div>
      ...
      {/* ══════════════════════════════════════════════════════════════════
          FEATURES — horizontal scroll cards with icon badges
      ══════════════════════════════════════════════════════════════════ */}
      <section id="features" className="pt-8 pb-4">
        <div className="flex gap-4 overflow-x-auto hide-scrollbar px-5 md:px-10 pb-2">
          {FEATURES.map((f) => (
            <div
              key={f.id}
              className="flex-none w-[260px] md:w-[290px] bg-[#0d0d0d] border border-white/8 rounded-2xl p-5 flex flex-col min-h-[400px]"
            >
              {/* Icon badge — matches tuyo's small colored square icons */}
              <div
              ...
              <p className="text-white font-display font-bold text-xl leading-tight mb-1.5">
                {f.headline}
              </p>
      ...
      {/* ══════════════════════════════════════════════════════════════════
          TESTIMONIALS — name/avatar at top (tuyo exact), infinite marquee
      ══════════════════════════════════════════════════════════════════ */}
      <section className="py-20 md:py-28 overflow-hidden">
        ...
        <div className="marquee-track overflow-hidden mb-3">
          <div className="animate-marquee flex gap-3" style={{ width: "max-content" }}>
            {[...TESTIMONIALS, ...TESTIMONIALS].map((t, i) => (
              <div key={i} className="flex-none w-[300px] bg-[#0d0d0d] border border-white/8 rounded-2xl p-5">
                {/* Name + avatar at top — matches tuyo */}
        ...
        <div className="marquee-track overflow-hidden">
          <div className="animate-marquee-slow flex gap-3" style={{ width: "max-content", animationDirection: "reverse" }}>
      ...
      {/* ══════════════════════════════════════════════════════════════════
          FEATURE CALLOUTS — two large cards side by side
          Image slots ready for user uploads
      ══════════════════════════════════════════════════════════════════ */}
      <section id="about" className="px-5 md:px-10 pb-5 grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Callout 1 — dark, booking */}
        <Link
          href="/#waitlist"
        ...
        >
          ...
          {/* Image slot — replace with callout-left.png when ready */}
          <div className="mt-auto pt-8 relative">
            {/* Placeholder until user uploads callout-left.png */}
            <div className="rounded-2xl overflow-hidden bg-black/50 border border-white/8">
              <div className="p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[#CCFF00] text-[10px] font-semibold border border-[#CCFF00]/30 rounded-full px-2 py-0.5">5 slots open</span>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  {["6 AM","7 AM","8 AM","9 AM","10 AM","11 AM"].map((t, i) => (
                    <div key={t} className={`text-center py-2 rounded-lg text-[10px] font-medium ${i===2 ? "bg-[#CCFF00] text-black" : i===4 ? "bg-white/5 text-white/15 line-through" : "bg-white/5 text-white/40"}`}>{t}</div>
                  ))}
                </div>
              </div>
            </div>
        ...
        >
          ...
          {/* Image slot — replace with callout-right.png when ready */}
      ...
      {/* ══════════════════════════════════════════════════════════════════
          CTA — centered, big headline + lime button
      ══════════════════════════════════════════════════════════════════ */}
      <section className="py-28 md:py-36 text-center px-5">
        <h2 className="font-display font-black text-[56px] xs:text-[72px] md:text-[100px] xl:text-[120px] leading-[0.88] tracking-tight text-white mb-6">
      ...
      {/* ══════════════════════════════════════════════════════════════════
          ...
          LEFT: hero text + store-style buttons (no form)
          RIGHT: FAQ accordion as individual dark rounded blocks
      ══════════════════════════════════════════════════════════════════ */}
      <section className="px-5 md:px-10 pb-24 grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-20">

        {/* Left — hero headline repeat + CTA buttons (like tuyo's App Store repeat) */}
        <div className="flex flex-col justify-between">
          <div>
            <h2 className="font-display font-black text-[56px] xs:text-[68px] md:text-[80px] leading-[0.88] tracking-tight text-white mb-5">
              Book your<br />
              <span className="text-[#CCFF00]">turf.</span>
            </h2>
            ...
            {/* Store-style dark buttons — like tuyo's App Store / Play Store */}
            <div className="flex items-center gap-3 flex-wrap mb-6">
              <Link
                href="/#waitlist"
                className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-[#111] border border-white/12 text-white text-sm font-semibold hover:bg-[#1a1a1a] transition-all duration-200"
              >
                <Image src="/turfinLogo.svg" alt="" width={14} height={14} className="object-contain" />
                Join Waitlist
              </Link>
              <Link
                href="/#features"
                className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-[#111] border border-white/12 text-white/60 text-sm font-semibold hover:bg-[#1a1a1a] hover:text-white transition-all duration-200"
              >
        ...
        {/* Right — FAQ as individual dark rounded blocks (tuyo exact style) */}
        <div className="flex flex-col gap-2 pt-0 md:pt-10">
          {FAQS.map((faq, i) => (
            <div key={i} className="bg-[#0d0d0d] border border-white/8 rounded-xl overflow-hidden">
              ...
              >
                <span className="text-white/75 text-sm font-medium pr-4 group-hover:text-white">
                  {faq.q}
                </span>
                <span
      ...
      {/* ══════════════════════════════════════════════════════════════════
          WAITLIST — standalone glassmorphic card (original design, only here)
      ══════════════════════════════════════════════════════════════════ */}
      <section
        id="waitlist"
        className="relative px-3 xs:px-4 py-16 md:py-24 flex justify-center overflow-hidden"
      >
        {/* Subtle lime radial glow behind the card */}
        <div
              ...
              <div className="flex flex-col md:flex-row items-center gap-3 md:gap-4 mt-1">
                <div className="flex -space-x-2.5">
                  {AVATARS.map(({ char, src, gradient }) => (
                    ...
                    >
                      {src ? (
                        <Image src={src} alt={char} width={36} height={36} className="object-cover w-full h-full" />
                      ) : (
                        <span className="text-[10px] md:text-xs font-bold text-white/80">{char}</span>
                      )}
                    </div>
                  ))}
                </div>
      ...
      {/* ══════════════════════════════════════════════════════════════════
          FOOTER — tuyo-style: logo + links row + legal text
      ══════════════════════════════════════════════════════════════════ */}
